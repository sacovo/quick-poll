...
def delegate_data(request):
    question: Question = (
        Question.objects.filter(closed=False).order_by("created").first()
    )
...
def create_question(request):
    ...
    options = [option.strip() for option in request.POST.get("options").split("\n")]

    secret = request.POST.get("secret") == "true"

    question = Question.objects.create(question=question, closed=False, secret=secret)
